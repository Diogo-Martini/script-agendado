...
# === API CONFIG ===
# Em vez do WSDL, chamamos direto o endpoint principal.
# Normalmente é o mesmo caminho sem o "?wsdl".
...
params_base = {
    ...
    # Parâmetro que indica qual operação chamar no "webservice".
    # Isso pode variar por instalação (METODO / metodo / ACAO / OPCAO ...).
    # Estamos assumindo METODO=ConsultarOcorrencias com base no nome do método usado no Zeep.
...
# === CHAMADA HTTP DIRETA ===
# Vamos tentar GET. Se o ambiente Multidados exigir POST,
# é só trocar por requests.post(API_URL, data=params, timeout=60)
resp = requests.get(API_URL, params=params, timeout=60)

if resp.status_code != 200:
    raise RuntimeError(f"Erro HTTP {resp.status_code} ao consultar API: {resp.text[:500]}")

# A API deveria retornar JSON válido porque RETORNO='json'
try:
...
if df_novo.empty:
    ...
    # === Limpeza e formatação de campos ===
    ...
    # força ordem de colunas como a planilha espera
